...
db_test = pd.read_csv("C:\\Users\\luan_\\Downloads\\desafio\\test.csv")
db_train = pd.read_csv("C:\\Users\\luan_\\Downloads\\desafio\\train.csv")
...
#Separação de caracteristicas e classe(Regressão)
y = a1['NU_NOTA_MT']
x = a1.drop(['NU_NOTA_MT'], axis=1)

#normalização do vetor de treino
...
#Substituir valores nulos pela media da linha
medb = b1.mean(axis=1)
...
#Normalizar caracteriscas do db_test

n = b1[['NU_INSCRICAO']]

b1 = b1.drop(['NU_INSCRICAO'], axis=1)

scaler_test = preprocessing.StandardScaler().fit(b1)
x_test_scale = scaler_test.transform(b1)
...
notas.to_csv(r'C:\\Users\\luan_\\Downloads\\desafio\\answer.csv', index = False, header=True)
